    ...
    private Button createButton(int row, int col) {
        ...
        button.setOnAction(e -> {
            ...
            if (!gameOver && board[row][col] == null) {
                ...
                if (checkWin(row, col)) {
                    endGamePopup.display("Game Over! \n Jugador " + (isPlayer1Turn ? "Segundo" : "Primero") + " Gana!");
...
private int evaluateThreeInRow(int row, int col, String playerSymbol) {
    ...
    count += (countAdjacentCells(row, col, 0, 1, playerSymbol) + countAdjacentCells(row, col, 0, -1, playerSymbol) == 2) ? 7 : 0;
    
    count += (countAdjacentCells(row, col, 1, 0, playerSymbol) + countAdjacentCells(row, col, -1, 0, playerSymbol) == 2) ? 7 : 0;
    
    count += (countAdjacentCells(row, col, 1, 1, playerSymbol) + countAdjacentCells(row, col, -1, -1, playerSymbol) == 2) ? 7 : 0;
    count += (countAdjacentCells(row, col, 1, -1, playerSymbol) + countAdjacentCells(row, col, -1, 1, playerSymbol) == 2) ? 7 : 0;
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (board[i][j] == null) {
                    ...
                    // moveQueue.offer(new Move(i, j, score)); 
